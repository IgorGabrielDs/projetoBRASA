(function () {
  ...
  // Alterna estilos conforme "salvo"
  function applySavedState(btn, saved, labelText) {
    btn.setAttribute('aria-pressed', saved ? 'true' : 'false');

    // Mantém compatibilidade com suas utilitárias
    ...
    if (label) {
      // usa o label do backend, ou um fallback semântico
  ...
  async function toggleSalvar(btn) {
    ...
    try {
      ...
      // --- NOVO: se "removeu dos salvos" e estamos na página /salvos, remover o card
      if (!data.saved) {
        const list = document.getElementById('saved-list');
        const article = btn.closest('article');
        if (list && article && list.contains(article)) {
          article.remove();
          // se ficou vazio, mostra a mensagem "vazio"
